...
C     En 2D
      real*8   shp2D(3,nnod),Nmat2D(1,nnod)
      real*8   Bmat2D(2,nnod),Nmatel2D(2,2*nnod),Bmatel2D(3*(dim-1)+axi,2*nnod)
      real*8   Bmatplano2D(3,2*nnod),r,r0,Bmataxi2D(3*(dim-1)+axi,2*nnod)
      real*8   Def2D(3*(dim-1)+axi)
C
C     Inicializacion de variables
      xjac	     =   0.d0
      chi	     =   0.d0
...
C
         open(UNIT=14,file=trim(filename), status='old')
            ! Read the line containing the stdWeight value
            read(14, '(A)') line
